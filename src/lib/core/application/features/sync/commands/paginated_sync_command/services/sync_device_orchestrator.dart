...
class SyncDeviceOrchestrator {
    ...
    try {
      ...
      for (int i = 0; i < configs.length; i++) {
        ...
        try {
          ...
          Logger.info('Using last sync date: $lastSyncDate for ${config.name}');
          ...
          Logger.error('CRITICAL: Exception during ${config.name} sync', error: e, stackTrace: stackTrace);
      ...
      Logger.error('CRITICAL: Exception in syncWithDevice for ${syncDevice.id}', error: e, stackTrace: stackTrace);
      return false;
    }
  }
  ...
  Future<void> _processPendingResponses({
    ...
    Logger.info('Processing ${pendingResponseData.length} pending response DTOs');
    ...
    Logger.info('Grouped into ${groupedResponses.length} entity types');
    ...
    for (final entry in groupedResponses.entries) {
      ...
      Logger.info('Processing ${responseDtos.length} pages of $entityType');
    ...
    Logger.info('Total processed: $totalProcessed, conflicts: $totalConflicts');
    _paginationService.clearPendingResponseData();

    final overallProgress = calculateOverallProgress();
    Logger.info('Overall progress: ${overallProgress.overallProgress.toStringAsFixed(1)}%');
  }
  ...
  Future<({int processed, int conflicts})> _processEntityResponse({
    ...
    try {
      ...
      Logger.info('Processed $processedCount items from $entityType ($pageCount pages)');
      return (processed: processedCount, conflicts: estimatedConflicts);
    } catch (e) {
      Logger.error('Failed to process $entityType: $e');
      final errorKey = _getErrorKey(e);
  ...
  String _getErrorKey(dynamic e) {
    if (e is SyncValidationException) {
      if (kDebugMode) {
        Logger.info('DEBUG: SyncValidationException! Code: ${e.code}, params: ${e.params}');
      }
