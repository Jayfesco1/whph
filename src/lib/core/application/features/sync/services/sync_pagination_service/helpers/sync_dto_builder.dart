...
class SyncDtoBuilder {
  ...
  PaginatedSyncDataDto _buildHabitDto(
    ...
    if (habitsData != null) {
      Logger.debug('Habit DTO - sample habit IDs: ${habitsData.data.createSync.take(3).map((h) => h.id).toList()}');
    }
  ...
  PaginatedSyncDataDto _buildHabitTagDto(
    ...
    if (habitTagsData != null) {
      Logger.debug('HabitTag DTO - sample tag IDs: ${habitTagsData.data.createSync.take(3).map((t) => t.id).toList()}');
    }
